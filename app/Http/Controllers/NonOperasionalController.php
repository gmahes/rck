    ...
    public function convertXlsToXlsx()
    {
        if (request()->invoiceImport == "true") {
            ...
            foreach ($attr['invoices'] as $invoice) {
                ...
                $sheet->setCellValue('B' . $row, $invoice['tanggal_invoice']);
                ...
                if (gettype($invoice['deskripsi']) == 'string') {
                    ...
                    $sheet->setCellValue('F' . $row, number_format($invoice['jumlah'], 2, ',', '.'));
                    $sheet->setCellValue('G' . $row, number_format($invoice['jumlah'] * 0.011, 2, ',', '.'));
                } else {
                    foreach (array_keys($invoice['deskripsi']) as $key) {
                        $sheet->setCellValue('D' . $row, $invoice['unit'][$key]);
                        $sheet->setCellValue('E' . $row, $invoice['deskripsi'][$key]);
                        $sheet->setCellValue('F' . $row, number_format($invoice['jumlah'][$key], 2, ',', '.'));
